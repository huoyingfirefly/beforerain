...
【非对称核素浓度】默认必须输出[核素-1]。你只能先假设场景安全，然后再找危险理由。以下三种情况才能输出正值：(1)主AI叙事明确提到暴雨/触雨/雨滴→[核素+3] (2)主AI明确写到敌人持武器攻击玩家→[核素+2] (3)主AI明确写到玩家受伤或身处险境→[核素+1]。如果不属于以上任何情况，必须输出[核素-1]或[核素-2]。浓度≥10时输出[冒险失败:死因]。
...
def build_messages(prompt: str, history: list[dict], system: str, rag_context: str = ""):
    ...
    # 每轮强制追加指令
    prompt = prompt + "\n\n[系统指令]1.必须严格按照世界观中的框架(A-E之一)展开剧情，禁止编造框架外的身份和场景 2.回复末尾必须包含---然后列出3个选项[/系统指令]"
