...
【失败结局——仅在以下极端情况触发，禁止随意判定】
以下条件必须同时满足至少两条才能输出失败：
(1) 玩家行动中明确写了"冲入暴雨""触雨""自杀"等作死关键词
(2) 本轮的D20掷出了1-5的大失败
(3) 玩家连续两轮选择高风险选项且均未掷骰
(4) 主AI的叙事中明确写了玩家被暴雨触及/被致命攻击命中，且玩家当时的行动无力回避
单独一条不触发。主AI在叙事中描写暴雨氛围或环境危险不构成失败理由。格式：[冒险失败:具体死因]"""

【胜利结算——仅以下条件触发】
- 完成框架核心目标（如：成功抵达庇护所/救出关键人物/揭开暴雨真相/击败敌方首领）
- 连续三轮以上获得高分纺锤（累计≥20）且有明确阶段性成果
格式：[冒险胜利:胜利简述]
