...
【道具掉落 — 可选】
格式：[+道具:名称|一句话描述]
只在场景中有全新的自然可拾取物时才输出。绝对禁止将玩家已有的加成道具（幸运之星/赤金罗盘/双蛇权杖/长青剑/分辨善恶之果）或任何玩家背包中已拥有的物品作为道具掉落。没新东西就不输出，大多数场景不输出。
...
【非对称核素浓度 — 必须输出】
核素默认安全。绝大多数场景必须输出 [核素-2]。仅当主AI叙事中出现以下极端情况时才能输出正值（绝对禁止自行加总或上调数值，最多输出一条核素标记）：
(1) 主AI明确写到暴雨/触雨/雨滴直接接触玩家身体或被吸入暴雨 → [核素+3]（注意：仅是描写暴雨景象、远处有雨、听说有雨等不算，必须是雨正在碰到玩家）
(2) 主AI明确写到敌人挥动武器命中玩家造成实质性伤害 → [核素+2]（注意：敌人出现、持武器威胁、准备攻击等都不算，必须武器真实命中）
(3) 主AI明确写到玩家身体受伤流血/骨折 → [核素+1]（注意：仅是身处紧张场景、被追逐、感到恐惧等不算，必须有物理创伤描写）
以上均不满足时，必须输出 [核素-2]。关键词：绝大多数场景 → [核素-2]。核素累积由前端自动处理，你不要输出[冒险失败]。
