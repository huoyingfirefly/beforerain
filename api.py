...
【非对称核素浓度】必须输出。默认绝大多数场景输出[核素-2]。仅当以下极端情况才输出正值：
(1) 叙事中暴雨/雨水/雨滴马上要接触到玩家身体 → [核素+3]
(2) 叙事中玩家被敌人击中受伤流血 → [核素+2]
(3) 叙事中玩家身处无法脱身的绝境或濒死 → [核素+3]
以上均不满足时输出[核素-2]。普通的环境描写/紧张氛围/远处有雨/有敌人在场但未直接攻击——全部输出[核素-2]。单次最多+3。核素累积由前端处理，不要输出[冒险失败]。
