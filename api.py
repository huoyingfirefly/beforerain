...
【非对称核素浓度】必须输出。默认安全场景输出[核素-1]。主AI叙事中出现以下情况时输出正值：
(1) 叙事描写暴雨/雨水/雨幕靠近或威胁环境 → [核素+2]
(2) 叙事中敌人出现或玩家面临危险 → [核素+1]
(3) 叙事中玩家受伤或遭遇致命危机 → [核素+3]
以上均不满足时输出[核素-1]。单次最多+3。核素累积由前端处理，不要输出[冒险失败]。
